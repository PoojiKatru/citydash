...
var parcels := {}  # Vector2i -> "." or "o"


func reset_parcels() -> void:
	parcels.clear()
	for y in CityData.ROWS:
		for x in CityData.COLS:
			var symbol: String = CityData.LAYOUT[y][x]
			if symbol == "." or symbol == "o":
				parcels[Vector2i(x, y)] = symbol
	queue_redraw()


# Returns "." or "o" if something was picked up here, "" otherwise.
func collect(cell: Vector2i) -> String:
	if not parcels.has(cell):
		return ""
	var kind: String = parcels[cell]
	parcels.erase(cell)
	queue_redraw()
	if parcels.is_empty():
		all_collected.emit()
...
func _draw() -> void:
	var t := float(CityData.TILE)
	draw_rect(Rect2(Vector2.ZERO, Vector2(CityData.COLS, CityData.ROWS) * t), ROAD)

	for y in CityData.ROWS:
		for x in CityData.COLS:
			var cell := Vector2i(x, y)
			match CityData.LAYOUT[y][x]:
				"#":
					_draw_building(cell)
				"=":
					_draw_shutter(cell)
	...
	for cell: Vector2i in parcels:
		if parcels[cell] == "o":
			_draw_coffee(CityData.cell_to_world(cell))
		else:
			_draw_parcel(CityData.cell_to_world(cell))
...
func _draw_lane_markings(cell: Vector2i) -> void:
	...
	if horizontal == vertical:
		return
	var centre := CityData.cell_to_world(cell)
...
func _is_building(cell: Vector2i) -> bool:
	if cell.x < 0 or cell.x >= CityData.COLS or cell.y < 0 or cell.y >= CityData.ROWS:
		return true
	return CityData.LAYOUT[cell.y][cell.x] == "#"


func _is_solid(cell: Vector2i) -> bool:
	if cell.x < 0 or cell.x >= CityData.COLS or cell.y < 0 or cell.y >= CityData.ROWS:
		return true
	var symbol: String = CityData.LAYOUT[cell.y][cell.x]
